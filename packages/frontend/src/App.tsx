...
const App: React.FC = () => {
  ...
  const {
    ...
    signedSubject,
    setIsLoading,
  } = useFetchGuilds(veraxSdk, address, new URLSearchParams(window.location.search).get('code'));
  ...
  const issueAttestation = useCallback(async (signedGuild: SignedGuild) => {
    if (address && veraxSdk && signedSubject) {
      try {
        let receipt = await veraxSdk.portal.attest(PORTAL_ID, {
          schemaId: SCHEMA_ID,
          expirationDate: Math.floor(add(new Date(), { months: 1 }).getTime() / 1000),
          subject: address,
          attestationData: [{ guildId: signedGuild.id }],
        }, [signedSubject, signedGuild.signature], false, 100000000000000n);
        if (receipt.transactionHash) {
          setTxHash(receipt.transactionHash);
          receipt = await waitForTransactionReceipt(wagmiConfig.getClient(), { hash: receipt.transactionHash });
  ...
  }, [address, veraxSdk, signedSubject]);

  const handleAttest = useCallback(async (signedGuild: SignedGuild) => {
    if (isConnected) {
      setTxHash(undefined);
  ...
  const handleCheck = useCallback((signedGuild: SignedGuild) => {
